...
*GENERATING AND LABELLING NEW VARIABLES
...
*HOW COULD YOU YOU CREATE A YEARS OF SCHOOLING VARIABLE FOR EVERYONE?

//first creating a variable for enrolled
gen enrolled = 0 if M02_SCHOOL_SCORRES==0 | M02_SCHOOL_SCORRES==2
replace enrolled = 1 if M02_SCHOOL_SCORRES==1

la variable enrolled "Ever enrolled"

label define yesno 1 yes 0 no

label define enroll 1 "ever enrolled" 0 "never enrolled"


label values enrolled enroll
...
//now creating a years of education variable.

gen years_enrolled = 0 if enrolled==0
replace years_enrolled = M02_SCHOOL_YRS_SCORRES if enrolled==1
la var years_enrolled "Years of schooling"
label define neverenrolled 0 "never enrolled"
label values years_enrolled neverenrolled


//What if we wanted to create an indicator that the respondent has more that
//seven years of education?

gen years_7plus = 0 if years_enrolled<7
replace years_7plus = 1 if years_enrolled>=7

gen years_7plus_v2 = 0 if years_enrolled<7
replace years_7plus_v2 = 1 if years_enrolled>=7 &years_enrolled!=.

*alternate method

gen years_7plus_v3 = years_enrolled>=7 if years_enrolled!=.

compare years_7plus_v2 years_7plus_v3
...
*****************setting the global variable list;

gl varlist M04_OBSSTDAT M04_ANC_N_1 M04_PATIENT_DSDECOD M04_DTHDAT ///
 ...
 //here i am defining a local macro that is the variable label
local lM04_OBSSTDAT : variable label M04_OBSSTDAT
local lM04_ANC_N_1 : variable label M04_ANC_N_1
local lM04_PATIENT_DSDECOD : variable label M04_PATIENT_DSDECOD
...
log close
 ...
 /* Demonstrating what to do if you only want to reshape a subset of variables
 
 keep MOMID PREGNANCYID M04_visit_num M04_OBSSTDAT M04_ANC_N_1 M04_PATIENT_DSDECOD M04_DTHDAT ///
 M04_PRG_DSDECOD M04_PRG_DSSTDAT M04_PRG_DSSTERM M04_HTN_EVER_MHOCCUR ///
 M04_DIABETES_EVER_MHOCCUR
 ...
 i(MOMID PREGNANCYID) j(M04_visit_num)
 */
*reshape long $varlist, i(MOMID PREGNANCYID) j(visit_num)
